...
- **Bundled MCP server (`mcp/`, zero-dependency).** New `.mcp.json` registers `node ${CLAUDE_PLUGIN_ROOT}/mcp/index.mjs`, exposing four deterministic tools — `verify_result_claim`, `query_graph`, `memory_graph_add`, `placeholder_scan` — plus a `toolkit://principles` resource. The server implements the MCP stdio protocol by hand (no SDK), so the GitHub-installed plugin runs with **no `npm install`**. Verified by `mcp/smoke-test.mjs` (stdio handshake + 4-tool list + tool call) and a functional test of the data tools.
- **`skills-generic/` grew from 12 to 25** with `session-watchdog`, `env-bootstrap`, and the thinking/style set: 10x, brief, godmode, scout, critique, devil, compare, pitch, teacher, explainlikeim5, humanizer, ooda.
- **Mandatory env isolation.** New `env-bootstrap` skill (uv-first `.venv` for Python, local `node_modules` for Node) plus a `block-global-pip` hook that refuses global `pip install` outside a venv, `sudo pip`, and `npm install -g`. A rule in `CLAUDE.md.template` makes per-project isolation the default so package/module versions never collide.
- **New `/toolkit-init` command** (`commands/`) bootstraps the project templates + the isolated env from inside Claude Code.
- **Plugin-active hooks.** New `hooks/hooks.json` wires the `templates/hooks/` guards (block-dangerous-bash, block-global-pip, format-on-write, session-briefing) so they fire in every project the plugin is enabled in — not only projects bootstrapped via `install.sh`.
- **HTML dashboard.** `dashboard/dashboard.html` (generated by zero-dep `dashboard/build.mjs`) — a self-contained, searchable visual catalog of every skill, agent, hook, MCP tool, and command. Open it in any browser; regenerate after changes with `node dashboard/build.mjs`.
- **New docs:** top-level [`ADD-TO-ANY-PROJECT.md`](ADD-TO-ANY-PROJECT.md) quickstart and [`docs/PLUGIN.md`](claude-toolkit/docs/PLUGIN.md) reference. `.gitignore` excludes `mcp/node_modules` and local `*.db` (ruvector) artefacts.
